...
class Tables
{
  ...
  /**
   * Build a Lum\DB\Schemata\Tables object.
   *
   ...
   *
   * The $db object must be a subclass of Lum\DB\PDO\Simple object that
   * includes the Lum\DB\PDO\Simple\NativeDB trait.
   *
  ...
  public function __construct ($db, $opts=[], $dirs=null)
  {
    if (isset($db))
    {
      ...
      {
        throw new \Exception(__CLASS__.": \$db must be a sub-class of \\Lum\\DB\\PDO\\Simple");
      }
      if (!is_callable([$db, 'source']))
      {
        throw new \Exception(__CLASS__.": \$db must use \\Lum\\DB\\PDO\\Simple\\NativeDB trait");
      }
